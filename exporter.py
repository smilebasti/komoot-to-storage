...
ERROR_MESSAGES = {
    ...
    'webdav_connection_failed': {
        'en': 'Could not connect to WebDAV server. Please check the URL.',
        'de': 'Verbindung zum WebDAV-Server fehlgeschlagen. Bitte überprüfe die URL.'
    },
    'webdav_auth_failed': {
        'en': 'WebDAV authentication failed. Please check username and password.',
        'de': 'WebDAV-Authentifizierung fehlgeschlagen. Bitte überprüfe Benutzername und Passwort.'
    },
    'webdav_upload_failed': {
        'en': 'Failed to upload files to WebDAV server.',
        'de': 'Upload auf WebDAV-Server fehlgeschlagen.'
    }
}
...
class KomootApi:
    ...
    def generate_gpx(self, tour: Dict[str, Any]) -> str:
        ...
        for coord in coordinates:
            ...
            # Convert timestamp to ISO format (UTC)
            timestamp = datetime.fromtimestamp(t / 1000, tz=timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')
...
def export_tracks(config: Dict[str, Any], lang: str = 'en') -> Union[str, bytes]:
    ...
    folder_name = _sanitize_folder_name(config.get('export_name', ''))
    logger.info("Exporting %d tracks to %s (folder: %s)", len(tracks), storage_type, folder_name or '(root)')
    ...
    if storage_type == 's3':
        ...
        if not WEBDAV_AVAILABLE:
            raise ImportError("WebDAV support requires webdavclient3. Install with: pip install webdavclient3")
        save_to_webdav(
            config.get('webdav_url'),
            config.get('webdav_username'),
            config.get('webdav_password'),
            config.get('webdav_path', ''),
            tracks,
            folder_name,
            lang=lang
        )
        return f"Exported {len(tracks)} tracks to WebDAV server." if lang == 'en' else f"{len(tracks)} Touren auf WebDAV-Server exportiert."

    elif storage_type == 'download':
        zip_bytes = save_to_zip(tracks, folder_name)
        return zip_bytes  # Returns bytes instead of string for download mode
...
def save_to_nfs(nfs_path: str, tracks: List[Dict], folder_name: Optional[str] = None, lang: str = 'en'):
    ...
    # Resolve the base path to prevent traversal
    base_path = os.path.realpath(nfs_path)
    ...
    # Ensure target stays within base path (prevent path traversal)
    target_dir = os.path.realpath(target_dir)
    if not target_dir.startswith(base_path):
        raise ValueError("Invalid export path: directory traversal detected")
    ...
    try:
        ...
        for track in tracks:
            ...
            file_path = os.path.realpath(os.path.join(target_dir, safe_name))

            # Double-check each file path stays within base
            if not file_path.startswith(base_path):
                logger.warning("Skipping file with traversal path: %s", track['name'])
                continue

            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(track['gpx_data'])

        logger.info("Saved %d tracks to NFS path: %s", len(tracks), target_dir)
...
def save_to_webdav(url: str, username: str, password: str, path: str,
                   tracks: List[Dict], folder_name: Optional[str] = None, lang: str = 'en'):
    """Save GPX files to WebDAV server (Nextcloud, ownCloud, etc.)"""
    if not all([url, username, password]):
        raise ValueError("WebDAV configuration incomplete: url, username, and password are required")
    ...
    try:
        ...
        if '401' in str(e) or 'Unauthorized' in str(e):
            raise ExportError('webdav_auth_failed', lang=lang)
        raise ExportError('webdav_connection_failed', str(e), lang=lang)
    except Exception as e:
        raise ExportError('webdav_connection_failed', str(e), lang=lang)
    ...
    try:
        ...
        # Upload each GPX file
        import tempfile
        for track in tracks:
            safe_name = sanitize_filename(track['name']) + '.gpx'
            remote_path = f"{target_path}/{safe_name}" if target_path else safe_name

            # webdavclient3 needs a local file to upload
            with tempfile.NamedTemporaryFile(mode='w', suffix='.gpx', delete=False, encoding='utf-8') as tmp:
                tmp.write(track['gpx_data'])
                tmp_path = tmp.name
        ...
        raise ExportError('webdav_upload_failed', str(e), lang=lang)
    except Exception as e:
        raise ExportError('webdav_upload_failed', str(e), lang=lang)


def save_to_zip(tracks: List[Dict], folder_name: Optional[str] = None) -> bytes:
    """Package GPX files into a ZIP archive and return as bytes"""
    buffer = io.BytesIO()
    with zipfile.ZipFile(buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
        for track in tracks:
            safe_name = sanitize_filename(track['name']) + '.gpx'
            if folder_name:
                safe_name = f"{folder_name}/{safe_name}"
            zf.writestr(safe_name, track['gpx_data'])
    buffer.seek(0)
